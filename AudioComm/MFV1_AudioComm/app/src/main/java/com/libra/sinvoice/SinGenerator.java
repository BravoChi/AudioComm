...
public class SinGenerator {
    ...
    public void gen(FreqPair freqPair, int duration) {
        if (STATE_START == mState) {
            //mGenRate = genRate;
            int freq1 = freqPair.getFreq1(freqPair);
            int freq2 = freqPair.getFreq2(freqPair);
            mDuration = duration;
            ...
            double per = (freq1 / (double) mSampleRate) * 2 * Math.PI;
            double per2 = (freq2 / (double) mSampleRate) * 2 * Math.PI;

            double d1 = 0;
            double d2 = 0;

            LogHelper.d(TAG, "Freq1: " + freq1+"; Freq2: "+freq2);
            if (null != mCallback) {
                mFilledSize = 0;
                ...
                if (null != buffer) {
                    for (int i = 0; i < totalCount; ++i) {
                        if (STATE_START == mState) {
                            int out = (int) (Math.sin(d1) * n/2) +(int) (Math.sin(d2) * n/2)+ 128;
