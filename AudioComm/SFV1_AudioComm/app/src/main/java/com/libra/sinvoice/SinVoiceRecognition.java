...
public class SinVoiceRecognition implements Record.Listener, Record.Callback, VoiceRecognition.Listener, VoiceRecognition.Callback {
    ...
    @Override
    public void onRecognition(int index1,int index2) {
        LogHelper.d(TAG, "recognition:" + index1+"; "+index2);
        if (null != mListener) {
            if (Common.START_TOKEN == index1) {
                mListener.onRecognitionStart();
            } else if (Common.STOP_TOKEN == index1) {
                mListener.onRecognitionEnd();
            } else if (index1 > 0 && index1 <= mMaxCodeIndex) {
                mListener.onRecognition(mCodeBook.charAt(index1 - 1));
            }
        }
    }
