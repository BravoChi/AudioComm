...
public class SinVoicePlayer implements Encoder.Listener, Encoder.Callback, PcmPlayer.Listener, PcmPlayer.Callback {
    ...
    private final static int DEFAULT_GEN_DURATION = 10;
    ...
    private boolean convertTextToCodes(String text) {
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            for (int i = 0; i < len; ++i) {
                ...
                if (index > -1&&index!=lastCode) {
                    mCodes.add(index + 1);
                    lastCode = index;
                    //mCodes.add(Common.INTERVAL);
                }
                else if(index>-1&&index==lastCode){
                    mCodes.add(Common.REPET);
                    lastCode = index;
                }
            ...
            if (ret) {
                if(len%2==0)
                    mCodes.add(Common.END_EVEN);
                else
                {
                    mCodes.add(Common.END_ODD);
                }
            }
        } else {
            ret = false;
